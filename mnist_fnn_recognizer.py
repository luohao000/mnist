...
# 模型调用
from mnist_fnn import FlexibleFNN, layer_sizes

class MNISTRecognizer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("FNN手写数字识别器")
        self.root.geometry("500x650")
        
        # 加载模型
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = FlexibleFNN(layer_sizes)
        try:
            # 确保模型状态字典正确加载到设备
            state_dict = torch.load("fnn_model.pth", map_location=self.device)
            self.model.load_state_dict(state_dict)
            self.model.to(self.device)
            self.model.eval()
            model_status = f"模型已加载 (设备: {self.device})"
        except FileNotFoundError:
            model_status = "警告: 未找到 fnn_model.pth 文件"
            messagebox.showwarning("模型文件", "未找到 fnn_model.pth 文件，请先训练模型")
        
        # 标题
        title_label = tk.Label(self.root, text="FNN手写数字识别器", 
                              font=('Arial', 16, 'bold'))
        ...
        test_btn = tk.Button(button_frame2, text="测试模型", command=self.test_model,
        ...
        tk.Label(result_frame, text="识别结果:", font=('Arial', 12, 'bold')).pack()
    ...
    def recognize_digit(self):
        try:
            ...
            # 转换为PyTorch张量并确保在正确设备上
            torch_tensor = torch.tensor(tensor, dtype=torch.float32).unsqueeze(0).flatten().unsqueeze(0).to(self.device)
            ...
            self.status_var.set(f"识别完成 - 预测数字: {prediction} (置信度: {confidence:.1%})")
    ...
    def preview_tensor(self):
        ...
        info_label = tk.Label(preview_window, 
                             text=f"形状: {tensor.shape}\n范围: {tensor.min():.3f} - {tensor.max():.3f}",
                             font=('Arial', 10))
        info_label.pack()
    ...
    def save_tensor(self):
        ...
        filename = f"handwritten_digit_{timestamp}.npy"
        
        np.save(filename, tensor)
        self.status_var.set(f"张量已保存到: {filename}")
    ...
    def test_model(self):
        try:
            ...
            result_text = tk.Text(test_window, height=8, width=35)
            ...
            progress_label.config(text="正在测试模型准确率...")
            test_window.update()
            ...
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(test_loader):
                    ...
                    # 确保数据格式正确 - FNN期望展平的输入
                    if data.dim() == 4:
                        data = data.view(data.size(0), -1)  # 展平为784维
                    ...
                    if batch_idx % 20 == 0:
                        progress_label.config(text=f"测试进度: {batch_idx * 64}/{len(test_dataset)}")
                        test_window.update()
            
            accuracy = correct / total
            
            progress_bar.stop()
            progress_label.config(text="测试完成!")
            
            result_info = f"""模型测试结果:
            
总测试样本: {total}
正确预测: {correct}
测试准确率: {accuracy:.4f} ({accuracy*100:.2f}%)

模型性能: {'优秀' if accuracy > 0.95 else '良好' if accuracy > 0.90 else '一般'}
"""
            
            result_text.insert(1.0, result_info)
            self.status_var.set(f"模型测试完成 - 准确率: {accuracy:.2%}")
            
        except Exception as e:
            messagebox.showerror("测试错误", f"模型测试过程中出现错误:\n{str(e)}")
